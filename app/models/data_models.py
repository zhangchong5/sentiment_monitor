...
class SearchResultItem(BaseModel):
    ...
    来源：Serper API 返回的搜索结果
    用途：存储原始搜索结果，供后续处理
    """
    title: str = Field(..., description="搜索结果标题", min_length=1)
    url: HttpUrl = Field(..., description="目标 URL")
    snippet: str = Field(..., description="Google 提供的摘要", min_length=1)
    position: int = Field(..., ge=1, description="搜索结果位置（1 开始）")
...
class ArticleContent(BaseModel):
    ...
    用途：存储抓取并清洗后的网页内容
    特点：包含原始 HTML（供调试）和清洗后的纯文本
    """
    title: str = Field(..., description="网页标题")
    url: HttpUrl = Field(..., description="网页 URL")
    raw_html: str = Field(..., description="原始 HTML 内容", exclude=True)
    cleaned_text: str = Field(..., description="清洗后的正文文本")
    is_valid: bool = Field(True, description="是否有效内容（非 PDF/乱码等）")
...
class AISummary(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {
                "summary": "报道指出某银行香港分行因反洗钱系统缺陷被金管局处罚 500 万港元",
                "sentiment": "负面",
...
class ReportSectionOneItem(BaseModel):
    ...
    url_title: str = Field(..., description="URL 标题（截断显示）")
    url: HttpUrl = Field(..., description="完整 URL")
    ai_summary: str = Field(..., description="AI 风险摘要")
...
class ReportSectionTwoItem(BaseModel):
    ...
    title: str = Field(..., description="网页标题")
    url: HttpUrl = Field(..., description="网页 URL")
    google_snippet: str = Field(..., description="Google 搜索摘要")
    content_summary: str = Field(..., description="AI 内容总结")
    sentiment: Literal["正面", "中性", "负面"] = Field(..., description="情感倾向")
    screenshot_path: Optional[str] = Field(None, description="截图文件路径")
    ...
    class Config:
        json_schema_extra = {
            "example": {
                ...
                "total_results": 20,
                "valid_articles": 15,
                "generation_time": "2026 年 02 月 10 日 14:30",
                "ai_enabled": True
            }
        }
